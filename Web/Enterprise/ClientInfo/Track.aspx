<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Track.aspx.cs" Inherits="Web.Enterprise.ClientInfo.Track" %>
...
<body style="overflow: auto; overflow-x: hidden; overflow-y: scroll;">
    ...
    <div>
        <table style="margin: 0 0 0 auto;">
            <tr>
                <td>
                    <input type="button" id="btnPrevious" value="上一个" class="btn1" runat="server" />
                </td>
                <td style="width: 6px;">
                    &nbsp;
                </td>
                <td>
                    <input type="button" id="btnNext" value="下一个" class="btn1" runat="server" />
                </td>
                <td style="width: 6px;">
                    &nbsp;
                </td>
                <td>
                    <mlb:Operate ID="Operate1" runat="server" ShowShare="false" />
                </td>
            </tr>
        </table>
    </div>
        ...
        <div id="Info" class="clientInfo">
            <table cellpadding="1" cellspacing="1" class="info">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <mlb:Property ID="Property2" runat="server" ShowLabelStyle="true" />
                    </td>
                    <td class="name">
                        网址：
                    </td>
        ...
        <div class="barinfo">
            ...
            <table>
                <tr>
                    ...
                    <td>
                        <input type="text" id="txtNextDate" class="Wdate txt1" style="width: 150px;" />
                    </td>
                    <td>
                        提前<select id="ddlMinute" class="ddl1" name="ddlMinute">
                    ...
                    <td>
                        <asp:DropDownList ID="ddlReType" class="ddl1" runat="server">
                        </asp:DropDownList>
                    </td>
                    <td>
                        <input type="button" id="btnNextSubmit" class="btn1" value="预约" />
                    </td>
                </tr>
            </table>
    ...
    <script type="text/javascript">
        var CONFIG={
            CIID:"<%=Request["ciid"] %>",
            PreviousID:"<%=PreviousID %>",//上一条编号
            NextID:"<%=NextID %>",//下一条编号
            Reservation:"<%=Reservation %>",//标识是否为预约名录
            UpdateURL:"/enterprise/handler/cihandler.ashx"
        };
        //--状态标识
        var aryStatus = [1, 0, 0, 0];
        function funLoadTalk(obj, i) {
            /// <summary>
            /// 加载话术
            /// </summary>
            $(obj).html("正在加载....");
            $(obj).load(URLPlusRandom("info/Talk.aspx"), function () {
                aryStatus[i] = 1;
            });
        }
        function funLoadQuestion(obj,i){
            ...
            /// </summary>
            $(obj).html("正在加载....");
            $(obj).load(URLPlusRandom("info/question.aspx?ciid=<%=Request["ciid"] %>#list"), function () {
                aryStatus[i] = 1;
            });
        }
        function funLoadStatusLog(obj, i) {
            ...
            /// </summary>
            $(obj).html("正在加载....");
            $(obj).load(URLPlusRandom("info/StatusLog.aspx?ciid=<%=Request["ciid"] %>#loglist"), function () {
                aryStatus[i] = 1;
            });
        }
        ...
        $(function () {
            ...
            $("#btnPrevious").click(function(){
                if(CONFIG.PreviousID>0){
                    location.href="Track.aspx?ciid="+CONFIG.PreviousID+"&s="+CONFIG.Reservation;
                }
            });
            //下一个
            $("#btnNext").click(function(){
                if(CONFIG.NextID>0){
                    location.href="Track.aspx?ciid="+CONFIG.NextID+"&s="+CONFIG.Reservation;
                }
            });
            //----------------------------基本信息区--------------------------------
            //判断名录名称是否存在
            $("#txtClientName").blur(function () {
                ...
                $.ajax({
                    ...
                    data: { act: "exists", name: name,ciid:CONFIG.CIID },
            ...
            $("#txtMobile").blur(function () {
                ...
                if (v.length > 0 && !$.IsMobile(v)) {//判断格式是否为手机格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom(CONFIG.UpdateURL + "?act=existscontact"),
                        data: { ciid: CONFIG.CIID, type: "1", value: v },
                        success: function (data) {
                            if(data.flag == "1")
                            {
                                $("#TipsContact").html("");
            ...
            $("#txtTel").blur(function () {
                ...
                if (v.length > 0 && !$.IsTel(v)) {//判断格式是否为手机格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom(CONFIG.UpdateURL + "?act=existscontact"),
                        data: { ciid: CONFIG.CIID, type: "2", value: v },
                        success: function (data) {
                            if(data.flag == "1")
                            {
                                $("#TipsContact").html("");
            ...
            $("#btnUpdateBase").click(function () {  
                ...
                var data = "ciid=" + CONFIG.CIID + "&clientname=" + clientname + "&zipcode=" + zipcode + "&address=" + address + "&linkman=" + linkman + "&position=" + position + "&email=" + email+ "&website="+ website + "&tel=" + tel + "&mobile=" + mobile + "&fax=" + fax + "&qq=" + qq +"&msn=" + msn + "&remark=" + remark + "&sourceid=" + sourceId + "&areaid=" + areaId + "&tradeid=" + tradeId+ "&t=" + Math.random();
                ...
                $.ajax({
                    type:"POST",
                    async:false,
                    url:URLPlusRandom(CONFIG.UpdateURL+"?act=updatebase"),
                    data:data,
                    success:function(data){
                        var msg = "";
                        if (data == "1") {
            ...
            $("#btnUpdateStatus").click(function () {
                ...
                data = "ciid=" + CONFIG.CIID + "&status=" + status+ "&t=" + Math.random();
                if (status == 2) {
                    var wishid = $("#ddlWish").val();
                ...
                //--提交修改
                $.post(URLPlusRandom(CONFIG.UpdateURL+"?act=updatestatus"), data, function (data) {
                    var msg = "";
                    if (data == "1") {
                        msg = "修改成功";
                        PopTip.Show(PopTip.Type.Succ, msg,false);
            ...
            $("#lookbar .baritem").click(function () {
                ...
                if (aryStatus[i] == 0) {//--未加载
                    ...
                    funLoadQuestion($("#divTab").children("div").eq(i), i);
                        ...
                        funLoadTalk($("#divTab").children("div").eq(i), i);
                    }
                    else if (i == 3) { //状态跟踪
            ...
            $("#btnLogSend").click(function () {
                ...
                var data = "ciid=" + CONFIG.CIID + "&date=" + date + "&info=" + info+ "&t=" + Math.random();
                $.post(URLPlusRandom(CONFIG.UpdateURL+"?act=exchange"), data, function (data) {
                    if (data.result == "1") {//成功
                        ...
                        $(objTr).find(".uinfo").text(data.info);
                        $(objTr).removeAttr("id");
                        $(objTr).fadeIn("slow");
                        $("#txtInfo").val("");
            ...
            $("#btnNextSubmit").click(function(){
                var time=$("#txtNextDate").val();
                if(time.length==0){
                    PopTip.Show(PopTip.Type.Error, "请正确的时间",false);
                    return false;
                }
                var data="ciid="+CONFIG.CIID+"&time="+time+"&minute="+$("#ddlMinute").val()+"&type="+$("#ddlReType").val()+ "&t=" + Math.random();
                $.post(URLPlusRandom(CONFIG.UpdateURL+"?act=reservation"),data,function(data){
                    //alert(data);
                    if(data=="1")
                    {
                        PopTip.Show(PopTip.Type.Succ, "提交成功",false);
