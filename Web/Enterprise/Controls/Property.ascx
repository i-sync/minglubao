<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Property.ascx.cs" Inherits="Web.Enterprise.Controls.Property" %>
<%if(ShowLabelStyle){ %>
    <table>
        <tr>
            <%if(SourceFlag){ %>
...
<%}else{ %>
    <table>
        <tr>
            ...
            <%}if(TradeFlag){ %>
            <td>行业：</td>
            <td><asp:DropDownList runat="server" ID="ddlTrade" CssClass="ddl1 w120"></asp:DropDownList></td>
            <%} if (AreaFlag)
              { %>
            <td>地区：</td>
            <td><asp:DropDownList runat="server" ID="ddlArea" CssClass="ddl1 w120"></asp:DropDownList></td>
            <%} %>
        </tr>
    </table>
