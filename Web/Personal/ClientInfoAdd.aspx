<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClientInfoAdd.aspx.cs" Inherits="Web.Personal.ClientInfoAdd" %>
        ...
        $(function () {
            ...
            $("#txtMobile").blur(function () {
                ...
                if (v.length > 0 && $.IsMobile(v)) {//判断格式是否为手机格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom("Handler/ClientInfo.ashx"),
                        data: { key:"existscontact",ciid: "0", type: "1", value: v },
            ...
            $("#txtTel").blur(function () {
                ...
                if (v.length > 0 && $.IsTel(v)) {//判断格式是否为电话格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom("Handler/ClientInfo.ashx"),
                        data: { key:"existscontact",ciid: "0", type: "2", value: v },
                        success: function (data) {
                            if (data.flag == "1") {
            ...
            $("#txtName").blur(function () {
                ...
                if ($.IsNullOrEmpty(clientName)) {
                    ...
                    $.ajax({
                        async: false,
                        type: "POST",
                        url: "Handler/ClientInfo.ashx",
                        data: { key: "exists", clientname: clientName, clientinfoid: 0 },
                        success: function (data) {
                            if (data == "0") {
                                data = "<span class='succ'>这个名称可以使用</span>";
                            } else {
                                data = "<span class='error'>这个名称已经使用，请换一个</span>";
                            }
                            $("#TipCode").html(data);
                        },
                        dataType: "text"
