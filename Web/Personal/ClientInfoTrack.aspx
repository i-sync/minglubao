<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClientInfoTrack.aspx.cs"
    Inherits="Web.Personal.ClientInfoTrack" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        var CONFIG={
            ID:"<%=Request["id"] %>",
            POST_URL:"Handler/ClientInfo.ashx"
        };
        //--状态标识
        var aryStatus = [1, 0, 0];
        ...
        function funLoadStatusLog(obj, i) {
            ...
            /// </summary>
            $(obj).html("正在加载....");
            $(obj).load(URLPlusRandom("info/StatusLog.aspx?id="+CONFIG.ID+"#loglist"), function () {
                aryStatus[i] = 1;
            });
        }
        ...
        function funStatus(v,status)
        {
            ...
            {                
                ...
                var scarpid = $("#ddlScarp").val();
                var oldscarpid=$("#hdScarp").val(); 
        ...
        $(function () {
            ...
            $("#txtClientName").blur(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: URLPlusRandom(CONFIG.POST_URL),
                    data: { key: "exists", clientname: name,clientinfoid:CONFIG.ID },
                    success: function (data) {
                        //ShowTips("#TipsCode", data.flag == "1", data.detail);
                        if(data=="0")
                        {
                            $("#TipsName").html("<span class='succ'>这个名称可以使用</span>");
                            $("#txtClientName").removeClass("error");
                        }
                        else
                        {
                            $("#TipsName").html("<span class='error'>这个名称已经使用，请换一个</span>");
            ...
            $("#txtMobile").blur(function () {
                ...
                if (v.length > 0 && !$.IsMobile(v)) {//判断格式是否为手机格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom(CONFIG.POST_URL),
                        data: {key:"existscontact",ciid: CONFIG.ID, type: "1", value: v },
                        success: function (data) {
                            if(data.flag == "1")
                            {
                                $("#TipsContact").html("");
            ...
            $("#txtTel").blur(function () {
                ...
                if (v.length > 0 && !$.IsTel(v)) {//判断格式是否为手机格式
                    ...
                    $.ajax({
                        type: "POST",
                        url: URLPlusRandom(CONFIG.POST_URL),
                        data: { key:"existscontact",ciid: CONFIG.ID, type: "2", value: v },
                        success: function (data) {
                            if(data.flag == "1")
                            {
                                $("#TipsContact").html("");
            ...
            $("#btnUpdateBase").click(function () {
                ...
                var data = "key=updatebase&id=" + CONFIG.ID + "&clientname=" + clientname + "&zipcode=" + zipcode + "&address=" + address + "&linkman=" + linkman + "&position=" + position + "&email=" + email + "&tel=" + tel + "&mobile=" + mobile + "&fax=" + fax +"&website="+website+"&qq="+qq+"&msn="+msn+ "&remark=" + remark + "&sourceid=" + sourceId + "&areaid=" + areaId + "&tradeid=" + tradeId +"&t="+Math.random();
                //--提交修改
                //标识是否修改成功
                var flag =false;
                $.ajax({
            ...
            $("#btnUpdateStatus").click(function () {
                ...
                data = "key=updatestatus&id=" + CONFIG.ID + "&status=" + status;
                if (status == 2) {
                    var wishid = $("#ddlWish").val();
                    ...
                    if (scrapid == undefined || scrapid == "") {
                        PopTip.Show(PopTip.Type.Tips,"请选择报废理由",false);
                        return false;
                    }
                    data = data + "&scrapid=" + scrapid;
                }
                //--提交修改
                $.post(URLPlusRandom(CONFIG.POST_URL), data, function (data) {
                    var msg = "";
                    if (data == "1") {
                        msg = "修改成功";
            ...
            $("#btnLogSend").click(function () {
                ...
                if($.IsNullOrEmpty(date)){
                    PopTip.Show(PopTip.Type.Tips,"请输入时间",false);
                    $("#txtWdate").focus();
                    return false;
                }else if ($.IsNullOrEmpty(info)) {
                    //alert("请输入沟通内容");
                    PopTip.Show(PopTip.Type.Tips,"请输入沟通内容",false);
                    $("#txtInfo").focus();
                    return false;
                }
                var data = "key=exchange&id=" + CONFIG.ID + "&date=" + date + "&info=" + info+"&t="+Math.random();
                $.post(URLPlusRandom(CONFIG.POST_URL), data, function (data) {
                    if (data.result == "1") {//成功
                        ...
                        $("#txtInfo").val("");
                        $("#txtWdate").val("");
                        //$(":radio[name='rblType']")[0].checked = true;
                        $("#tbExchangeList tr").each(function (i) {
                            $(this).find("td").first().text(i);
                        });
                    }
                    else {
                        PopTip.Show(PopTip.Type.Error,"提交失败" + data,false);
                    }
                }, "json");
            });
        });
...
<body>
    ...
    <table style="width: 100%; margin: 0; font-size: 14px;">
        <tr>
            <td align="right">
                <mlb:Operate ID="Operate1" runat="server" ShowDelete="false" />
            </td>
        </tr>
    </table>
    <div style="height: 22px; line-height: 18px; text-indent: 20px; border-bottom: 1px solid #ccc;">
        <span style="font-size: 16px; font-weight: bold;">基本资料</span> 
        <span style="margin-left: 15px;"><a href="javascript:void(0)" id="switch">编辑</a></span>
    </div>
    <div id="divInfo" class="clientInfo" style="line-height: 34px; ">
        ...
        <div id="Edit" class="clientInfo" style="line-height: 34px; display: none">
            <table cellpadding="1" cellspacing="1">
                ...
                <tr>
                    ...
                    <td colspan="4">
                        <asp:TextBox runat="server" ID="txtRemark" CssClass="txt" Width="500" />
                    </td>
                    <td>
            ...
            <table>
                <tr>
                    ...
                    <td valign="top">
                        ...
                        <asp:HiddenField runat="server" ID="hdScarp" />
                    </td>
                </tr>
            </table>
        </div>
    ...
    <div class="updown">
        <img src="/images/up.jpg" id="imgUpDown" alt="关闭" />
    </div>    
    <div id="lookbar">
    ...
    <div id="divTab" class="barInfoList" style="margin-bottom: 10px;">
        <div class="barinfo" style="display: block;">            
            <table>  
                <tr>
