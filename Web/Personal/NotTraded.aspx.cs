...
namespace Web.Personal
{
    public partial class NotTraded : MLMGC.Security.PersonalPage
    {
        ...
        protected void databind()
        {
            ...
            //绑定意向程度
            ddlNotTrade.DataSource = new T_NotTraded().GetList(new E_NotTraded() { PersonalID = PersonalID });
            ...
            rpList.DataSource = new T_ClientInfo().NotTradeSelect(data);
            rpList.DataBind();
        ...
        {
            string url = string.Format(Request.Url.AbsolutePath + "?name={0}&nottradeid={1}&startdate={2}&enddate={3}{4}",txtName.Text,ddlNotTrade.SelectedValue,txtStartDate.Value,txtEndDate.Value, Property1.Query);
